    ...
    while stack:
        ...
        for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= ny < h and 0 <= nx < w and not visited[ny, nx]:
                ...
                if not is_similar(pixel_color, [0, 0, 0], tolerance=20) and is_similar(pixel_color, reference_color, tolerance):
                    stack.append((ny, nx))
                    visited[ny, nx] = True
...
if __name__ == "__main__":
    ...
    gif_path = STANDARD_GIFS["Fox"]  # Загружаем лисичку
    fps = 18
    size_value = 225  # например, 300px размер
    width = height = size_value

    # Если хочешь удалить фон перед запуском
    ...
    try:
        ...
        print(f"Не удалось обработать GIF: {str(e)}")
